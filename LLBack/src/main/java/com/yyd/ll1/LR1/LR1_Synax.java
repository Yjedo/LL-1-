package com.yyd.ll1.LR1;

import com.yyd.ll1.enity.LRData;
...
public class LR1_Synax {
    ...
    public static List<List<String>> returnTable; //分析表

    public Object run(String GS, String str) {
        ...
        if (util.Judge(list)) {
            ...
            util.show();
            // String s = "aabab#";
            util.AnalyStack("i+i*i#");
        }
        System.out.println("----------------------------------------------");
        System.out.println(Data.data.toString());
        System.out.println(util.pu);
        LRData lrData = new LRData();
        lrData.data = Data.data;
        lrData.table = returnTable;
//        Data.data.clear();
//        returnTable.clear();
        return lrData;
    }
    ...
    public static void main(String[] arg) throws IOException {
        ...
        if (util.Judge(list)) {
            ...
            //根据初始文法列表构建文法符号集(list2可能缺少了某些符号或多了某些符号)
            util.Util(list);
            //构建文法符号集
            util.Get_colt();
...
//结构体
class node {          //用于存放产生式，方便查找产生式的左部与右部
    public char left;
    public String right;
...
//对分析过程信息进行封装
class Data {
    public static List<ResultData> data;
}

//文件读取类
class FilereadUtil {
    public List<String> Read() throws IOException {
        BufferedReader in = new BufferedReader(new FileReader("src/main/resources/static/test1.txt"));
        String s;
        List<String> list = new ArrayList<>();
    ...
    public List<String> Read(String GS) throws IOException {
        List<String> list = new ArrayList<>();
        Arrays.stream(GS.split("\n")).forEach((String s) -> list.add(s));
        return list;
    }
}
...
class SyntaxUtil extends LR1_Synax {
    ...
    public void Closure(String str) {
        ...
        if (flag == 0) {        //如果不在，添加
            ...
            int temp2 = CLOSURE.get(CLOSURE.size() - 1).
                    St.get(CLOSURE.get(CLOSURE.size() - 1).St.size() - 1).indexOf('.');   //确定'.'的位置
            int temp3 = CLOSURE.get(CLOSURE.size() - 1).
                    St.get(CLOSURE.get(CLOSURE.size() - 1).St.size() - 1).indexOf(',');   //确定','的位置
            char ch1 = CLOSURE.get(CLOSURE.size() - 1).
                    St.get(CLOSURE.get(CLOSURE.size() - 1).St.size() - 1).charAt(temp2 + 1); //记录'.’后面的字符
            char ch2 = CLOSURE.get(CLOSURE.size() - 1).
                    St.get(CLOSURE.get(CLOSURE.size() - 1).St.size() - 1).charAt(temp2 + 2); //记录'.'后面第二个的字符
            String temp4 = CLOSURE.get(CLOSURE.size() - 1).
                    St.get(CLOSURE.get(CLOSURE.size() - 1).St.size() - 1).substring(temp3 + 1); //保存当前产生式的展望符
            if (IsNotsymbols(ch1)) {      //当点后面的是非终结符
                if (ch2 == ',') {          //如果非终结符后面没有字符
                    ...
                    if (flag1 == 0) {            //如果没有$，对于每一个新发现产生式的展望符设定为该first集
                        for (int i = 0; i < analy.size(); i++) {
                            if (analy.get(i).left == ch1) {
    ...
    public void AnalyStack(String s) {
        ...
        while (sur_str.size() > 0) {
            ...
            resultData1.step = Integer.toString(in);
            ...
            resultData1.stackData = temp.replace("|", "");
            ...
            if (st.charAt(0) == 's') {
                ...
                int x = int_stack.get(int_stack.size() - 1);
                action += "ACTION[" + in4 + "," + ch + "]=S" + x + ", 状态" + x + "入栈";
            }
            if (st.charAt(0) == 'r') {
                String tt = String.valueOf(st.charAt(1));
                ...
                char chTemp = analy.get(in5).left;
                char_stack.add(chTemp);
                for (int i = 0; i < colt.size(); i++) {
                    if (char_stack.get(char_stack.size() - 1) == colt.get(i)) {
                        in2 = i;
                    }
                }
                int tempInt = Integer.parseInt(PTable[int_stack.get(int_stack.size() - 1)][in2]);
                int_stack.add(tempInt);
                System.out.print(format(analy.get(in5).left + "->" + analy.get(in5).right + "规约"));
                pu += format(analy.get(in5).left + "->" + analy.get(in5).right + "规约");

                action = "r" + (in5 + 1) + ":" + analy.get(in5).left + "->" + analy.get(in5).right + "规约" +
                        ",GOTO(" + (int_stack.size() - 2) + "," +
                        analy.get(in5).left + ")=" + tempInt + " 入栈";
            }
            resultData1.action = action;
            Data.data.add(resultData1);
            ...
            if (PTable[int_stack.get(int_stack.size() - 1)][in3].equals("Acc") && (cc == non_colt.get(0)) && (ss == '#')) {
                ...
                pu += format(temp1);//
                resultData2.stackData = temp1.replace("|", "");
                temp1 = "";
                for (int j = 0; j < char_stack.size(); j++) {
                ...
                resultData2.action = "ACC: 分析成功";
                resultData2.s = temp1;
                Data.data.add(resultData2);
                break;
            }
            in += 1;
            System.out.println();
...
//        returnTable.add(list);
        ...
        for (int i = 0; i < CLOSURE.size(); i++) {
            ...
            for (int j = 0; j < colt.size(); j++) {
                ...
                pu += "\t" + PTable[i][j];
                if (!PTable[i][j].equals("!")) {
                    list1.add(PTable[i][j]);
                } else {
                    list1.add(" ");
                }
            }
//            returnTable.add(list1);
            System.out.println();
            pu += "\n";
        }
    }
